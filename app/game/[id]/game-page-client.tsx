...
function getFpsColor(preset: Preset): string {
  ...
  if (preset.fsrVersion && preset.fsrVersion !== "none") return "text-orange-400"
  return "text-text/60"
}
...
export function GamePageClient({
  ...
  const filteredStats = useMemo(() => {
    ...
    const filterUpscaler = (arr: StatsResponse["upscalerStats"]) =>
      arr.filter((item) => {
        ...
        if (filters.fsr !== "all" && item.fsrVersion !== filters.fsr) return false
        if (filters.frameGen !== "all" && item.frameGenMethod !== filters.frameGen)
          return false
        return true
      })
    ...
    return {
      ...
      historical: stats.historical
        .map((h) => ({
          ...h,
          entries: h.entries.filter((e) => deviceSet.has(e.hardwareSlug)),
        }))
  ...
  const filteredPresets = useMemo(() => {
    return presets.filter((p) => {
      if (selectedDevices.length > 0 && !selectedDevices.includes(p.hardwareSlug))
        return false
      if (filters.proton !== "all" && p.protonVersion !== filters.proton) return false
      if (filters.os !== "all" && p.osVersion !== filters.os) return false
      if (filters.fsr !== "all" && p.fsrVersion !== filters.fsr) return false
      if (filters.frameGen !== "all" && p.frameGenMethod !== filters.frameGen)
        return false
      return true
    })
        ...
        <div className="max-w-7xl mx-auto flex gap-4 sm:gap-6">
          ...
          <div className="flex flex-col gap-2 min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-2 mt-1">
              <Badge icon={TrendingUpIcon} value={counts.benchmarks} label="Benchmarks" />
              <Badge icon={SettingsIcon} value={counts.presets} label="Presets" />
              <Badge icon={MessageSquareIcon} value={counts.comments} label="Comments" />
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row gap-6">
          ...
          <div className="flex-[2]">
            ...
            {game.description ? (
              ...
              <p className="text-sm text-text/40 italic">No description available.</p>
            )}
          </div>
          ...
          <div className="flex-1">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <MetaItem label="Last Sync" value={formatDate(game.lastSync)} />
              ...
              <div className="mt-4">
                ...
                <div className="flex flex-col gap-2">
                  {platformSupport.map((ps) => (
                    <div
                      key={ps.id}
                      className="flex items-center justify-between p-2 rounded-md border border-border bg-text/[0.03]"
                    >
        ...
        <div className="max-w-7xl mx-auto flex flex-col gap-4">
          ...
          {stats && stats.deviceBreakdown.length > 0 && (
            <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-hide">
              {stats.deviceBreakdown.map((device) => {
                const active = selectedDevices.includes(device.hardwareSlug)
                return (
                  <button
                  ...
                  >
                    {device.hardwareName}
                    <span className="ml-1 text-text/40">({device.count})</span>
                  </button>
                )
              })}
            </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <FilterSelect
              ...
              options={["all", ...(stats?.filterOptions.protonVersions ?? [])]}
              onChange={(v) => setFilters((f) => ({ ...f, proton: v }))}
            />
            <FilterSelect
              label="SteamOS Version"
              value={filters.os}
              options={["all", ...(stats?.filterOptions.osVersions ?? [])]}
              onChange={(v) => setFilters((f) => ({ ...f, os: v }))}
            />
            <FilterSelect
              label="FSR Version"
              value={filters.fsr}
              options={FSR_OPTIONS.map((o) => o.toLowerCase())}
              onChange={(v) => setFilters((f) => ({ ...f, fsr: v }))}
            />
            <FilterSelect
              ...
              onChange={(v) => setFilters((f) => ({ ...f, frameGen: v }))}
            />
          </div>
        </div>
      </motion.div>
        ...
        <div className="max-w-7xl mx-auto flex flex-col gap-6">
          ...
          {filteredStats && (
            <div className="flex flex-col lg:flex-row gap-4">
              <div className="flex-[3] rounded-xl border border-border bg-text/[0.03] p-4">
                <h3 className="text-sm font-medium text-text/80 mb-2">
                  Historical Performance
                </h3>
                {filteredStats.historical.length > 0 ? (
                  <HistoricalAreaChart data={filteredStats.historical} />
                ) : (
                  <div className="h-[280px] flex items-center justify-center text-sm text-text/40">
                    No historical data
                  </div>
                )}
              </div>
              <div className="flex-1 flex flex-col gap-3">
                <StatCard
                  ...
                  value={filteredStats.summary.avgFps?.toFixed(1) ?? "—"}
                  icon={TrendingUpIcon}
                />
                <StatCard
                  label="Best Device"
                  value={filteredStats.summary.bestDevice ?? "—"}
                  icon={Gamepad2Icon}
                />
                <StatCard
                  label="Total Entries"
                  value={String(filteredStats.summary.totalEntries)}
                  icon={DatabaseIcon}
                />
              </div>
            </div>
          ...
          {/* Row 2 — Upscaler Bar */}
          {filteredStats && filteredStats.upscalerStats.length > 0 && (
            <div className="rounded-xl border border-border bg-text/[0.03] p-4">
              <h3 className="text-sm font-medium text-text/80 mb-2">
                Upscaler Performance
              </h3>
              <UpscalerBarChart data={filteredStats.upscalerStats} />
            </div>
          )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              {filteredStats.fpsRange.length > 0 && (
                <div className="rounded-xl border border-border bg-text/[0.03] p-4">
                  ...
                  <FpsRangeChart data={filteredStats.fpsRange} />
                </div>
              )}
              {filteredStats.deviceBreakdown.length > 0 && (
                <div className="rounded-xl border border-border bg-text/[0.03] p-4">
                  ...
                  <DeviceDonut data={filteredStats.deviceBreakdown} />
                </div>
              )}
            </div>
          )}
      ...
      >
        <div className="max-w-7xl mx-auto flex flex-col gap-4">
          ...
          {filteredPresets.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 gap-3 rounded-xl border border-border bg-text/[0.02]">
              ...
              {filteredPresets.map((preset) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* FPS */}
                    {preset.fpsAvg !== null && (
                      <div className={`text-sm tabular-nums ${fpsColor}`}>
                        <span className="font-semibold">{preset.fpsAvg}</span>
                    ...
                    <div className="flex flex-wrap items-center gap-1.5">
                      {preset.fsrVersion && preset.fsrVersion !== "none" && (
                    ...
                    <div className="flex flex-wrap items-center gap-2 text-[10px] text-text/40">
                      {preset.protonVersion && (
                        <span>Proton {preset.protonVersion}</span>
                      )}
                      {preset.osVersion && (
                        <span>SteamOS {preset.osVersion}</span>
                      )}
                    </div>
                  </div>
                )
...
function StatCard({
  ...
  return (
    <div className="flex items-center gap-3 p-4 rounded-xl border border-border bg-text/[0.03]">
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <select
        ...
        className="text-xs bg-background border border-border rounded-md px-2 py-1 text-text/80 focus:outline-none focus:border-primary min-w-[100px]"
      >
        {options.map((opt) => (
